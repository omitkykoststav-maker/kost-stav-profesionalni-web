...
const RECIPIENT = "omitkykoststav@gmail.com";
const THANK_YOU_URL = "https://www.kost-stav.cz/dekujeme.html";
const DEFAULT_SUBJECT = "Nova poptavka z webu KOST STAV";
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({ ok: true, redirect: THANK_YOU_URL });
  } catch (error) {
...
async function sendContactEmail(fields, files) {
  const message = buildEmail(fields, files);
  ...
  throw new Error("Missing email transport. Set RESEND_API_KEY or SMTP_HOST on Vercel.");
}

function buildEmail(fields, files) {
  const text = buildTextBody(fields, files);
  const replyTo = getFirst(fields, "email");
  const subject = DEFAULT_SUBJECT;
  const from = process.env.CONTACT_FROM || process.env.SMTP_FROM || process.env.SMTP_USER || RECIPIENT;
  const to = process.env.CONTACT_TO || RECIPIENT;

  return { from, to, subject, text, replyTo, files };
}

function buildTextBody(fields, files) {
  const labels = {
    ...
    typ_prace: "Typ prace",
    "typ_omitky[]": "Typ omitky",
    "typ_fasady[]": "Typ fasady",
    stav_objektu: "Stav objektu",
    plocha: "Priblizna plocha v m2",
    termin: "Pozadovany termin",
    popis: "Popis zakazky",
    souhlas: "Souhlas",
  };

  const lines = [
    "Nova poptavka z webu KOST STAV",
    "",
    "Cilovy e-mail: " + RECIPIENT,
    "",
  ];
